...
func NewACL(aclConfig *ACLConfig) (*ACLFilter, error) {
	acl := &ACLFilter{}
	if aclConfig == nil {
		return acl, nil
	}

	if len(aclConfig.AllowPeers) > 0 {
		acl.allowPeers = make(map[peer.ID]struct{})
		for _, s := range aclConfig.AllowPeers {
			p, err := peer.Decode(s)
			if err != nil {
	...
	if len(aclConfig.AllowSubnets) > 0 {
		acl.allowSubnets = make([]*net.IPNet, 0, len(aclConfig.AllowSubnets))
		for _, s := range aclConfig.AllowSubnets {
